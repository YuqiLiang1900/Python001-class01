...
from scrapy import signals
from maoyan.settings import USER_AGENT_LIST
...
class RandomHttpProxyMiddleware(HttpProxyMiddleware):
    ...
    def from_crawler(cls, crawler):
        if not crawler.settings.get('HTTP_PROXY_LIST'):
            raise NotConfigured('HTTP_PROXY_LIST not set or empty.')
...
class RotateUserAgentMiddleware(object):
    ...
    def from_crawler(cls, crawler):
        ...
        ua = cls(user_agents)
        crawler.signals.connect(ua.spider_opened, signal=signals.spider_opened) 
        return ua
